...
export interface ComponentOptionsBase<
  Props,
  RawBindings, // setup 返回值的原始绑定类型
  D, // 选项式 API 的 data 类型
  C extends ComputedOptions, // 计算属性类型
  M extends MethodOptions, // 方法类型
  Mixin extends ComponentOptionsMixin,
  Extends extends ComponentOptionsMixin,
  E extends EmitsOptions, // 自定义事件类型
  EE extends string = string,
  Defaults = {},
  I extends ComponentInjectOptions = {},
  II extends string = string,
  S extends SlotsType = {}, // 插槽类型
  LC extends Record<string, Component> = {}, // 子组件类型
  Directives extends Record<string, Directive> = {}, // 自定义指令类型
...
>
  ...
  // 组件的组合式 API 入口
  setup?: (
    this: void, // setup 内部 this 指向 undefined（避免开发者误用 this）
    props: LooseRequired<
      Props &
        Prettify<
          UnwrapMixinsType<
        ...
        >
    >,
    // 包含 emit、slots、attrs 等上下文
    ctx: SetupContext<E, S>,
  ) => Promise<RawBindings> | RawBindings | RenderFunction | void
  name?: string // 组件名称
  // 组件模板，支持字符串模板（如 <div>{{ msg }}</div>）或直接传入 DOM 节点（运行时编译场景）；
  // 注意：单文件组件（SFC）的模板会被编译器处理，不会直接出现在这个属性中。
  ...
  // type.
  render?: Function // 组件的渲染函数，优先级高于 template
  ...
  components?: LC & Record<string, Component> // 注册子组件
  ...
  // 声明组件的自定义事件，支持对象（带校验）或数组（仅声明事件名）
  emits?: (E | EE[]) & ThisType<void>
  slots?: S // 插槽
  expose?: Exposed[] // 暴露给父组件的属性
  ...
  /**
   * 标记 SSR 渲染函数是否内联在 setup 中返回（编译器标记）
  ...
  /**
   * 异步组件的加载器函数（defineAsyncComponent 生成）
   * marker for AsyncComponentWrapper
   * @internal
   */
  __asyncLoader?: () => Promise<ConcreteComponent>
  /**
   * 异步组件加载完成后，存储解析后的真实组件
   * the inner component resolved by the AsyncComponentWrapper
   * @internal
   */
  __asyncResolved?: ConcreteComponent
  /**
   * 用于懒水化（lazy hydration），SSR 场景下的异步组件 hydration 逻辑
...
export type ComponentOptions<
  ...
  // 第一步：继承 ComponentOptionsBase 的所有类型
> = ComponentOptionsBase<
  ...
  Provide
> &
  // 第二步：扩展 ThisType，约束选项式 API 中 this 的类型
  //  ThisType 是 TypeScript 提供的「上下文类型工具」
  ThisType<
